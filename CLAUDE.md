...
Use the focused agent when work clearly belongs to one area. Definitions live in `.claude/agents/`:
...
Detailed conventions live in `.claude/rules/` and apply to the matching paths:

- `.claude/rules/project-context.mdc` — core project context (always applies).
- `.claude/rules/git.md` — submodule-aware Git workflow (always applies).
- `.claude/rules/jira.md` — Jira/`acli` workflow on project `HFT` (always applies).
- `.claude/rules/python.md` — Python/FastAPI/MongoDB conventions for `backend/`.
- `.claude/rules/frontend.md` — TypeScript/React/MapLibre conventions for `frontend/`.
