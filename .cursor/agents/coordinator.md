...
When spawning a sub-agent that will edit code inside `backend/` or `frontend/`, **always request worktree isolation** so the sub-agent works on an isolated copy of the submodule and the user's working tree is not disturbed.
...
2. **Decompose if needed.** Break cross-cutting work into per-area sub-tasks. Independent sub-tasks should be spawned in parallel.
