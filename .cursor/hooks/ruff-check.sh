#!/usr/bin/env bash
# afterFileEdit hook: run Ruff on edited Python files when this is a uv-backed
# Python project. Missing tools, non-Python files, and absent pyproject files are
# treated as no-ops so early project setup is not blocked.
