...
Before you read code, plan, or edit anything, read `.claude/rules/python.md`. It is the authoritative source for Python 3.13+ typing, async IO, Pydantic v2, MongoDB collection design, risk-rule structure, Google-style docstrings, and Ruff configuration. Every line you write must comply. If you cannot match a rule by mirroring an existing pattern, stop and escalate to `backend-senior` via the coordinator rather than guessing.
